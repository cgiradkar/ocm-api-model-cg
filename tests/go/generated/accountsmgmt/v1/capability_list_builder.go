...
func (b *CapabilityListBuilder) Copy(list *CapabilityList) *CapabilityListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*CapabilityBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewCapability().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'capability' objects using the
// configuration stored in the builder.
func (b *CapabilityListBuilder) Build() (list *CapabilityList, err error) {
	items := make([]*Capability, len(b.items))
