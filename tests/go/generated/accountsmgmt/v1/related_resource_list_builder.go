...
// RelatedResourceListBuilder contains the data and logic needed to build
// 'related_resource' objects.
type RelatedResourceListBuilder struct {
	items []*RelatedResourceBuilder
}
...
func (b *RelatedResourceListBuilder) Copy(list *RelatedResourceList) *RelatedResourceListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*RelatedResourceBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewRelatedResource().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'related_resource' objects using the
// configuration stored in the builder.
func (b *RelatedResourceListBuilder) Build() (list *RelatedResourceList, err error) {
	items := make([]*RelatedResource, len(b.items))
