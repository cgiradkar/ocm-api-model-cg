...
type ClusterMetricsNodesListBuilder struct {
	items []*ClusterMetricsNodesBuilder
}
...
func (b *ClusterMetricsNodesListBuilder) Copy(list *ClusterMetricsNodesList) *ClusterMetricsNodesListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*ClusterMetricsNodesBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewClusterMetricsNodes().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'cluster_metrics_nodes' objects using the
// configuration stored in the builder.
func (b *ClusterMetricsNodesListBuilder) Build() (list *ClusterMetricsNodesList, err error) {
	items := make([]*ClusterMetricsNodes, len(b.items))
	...
	list = new(ClusterMetricsNodesList)
	list.items = items
	return
}
