...
// QuotaAuthorizationResponseListBuilder contains the data and logic needed to build
// 'quota_authorization_response' objects.
type QuotaAuthorizationResponseListBuilder struct {
	items []*QuotaAuthorizationResponseBuilder
}
...
func (b *QuotaAuthorizationResponseListBuilder) Items(values ...*QuotaAuthorizationResponseBuilder) *QuotaAuthorizationResponseListBuilder {
	b.items = make([]*QuotaAuthorizationResponseBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *QuotaAuthorizationResponseListBuilder) Copy(list *QuotaAuthorizationResponseList) *QuotaAuthorizationResponseListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*QuotaAuthorizationResponseBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewQuotaAuthorizationResponse().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'quota_authorization_response' objects using the
// configuration stored in the builder.
func (b *QuotaAuthorizationResponseListBuilder) Build() (list *QuotaAuthorizationResponseList, err error) {
	items := make([]*QuotaAuthorizationResponse, len(b.items))
	...
	list = new(QuotaAuthorizationResponseList)
	list.items = items
	return
}
