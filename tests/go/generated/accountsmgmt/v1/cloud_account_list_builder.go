...
func (b *CloudAccountListBuilder) Copy(list *CloudAccountList) *CloudAccountListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*CloudAccountBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewCloudAccount().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'cloud_account' objects using the
// configuration stored in the builder.
func (b *CloudAccountListBuilder) Build() (list *CloudAccountList, err error) {
	items := make([]*CloudAccount, len(b.items))
