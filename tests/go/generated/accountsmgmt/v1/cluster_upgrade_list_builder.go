...
// ClusterUpgradeListBuilder contains the data and logic needed to build
// 'cluster_upgrade' objects.
type ClusterUpgradeListBuilder struct {
	items []*ClusterUpgradeBuilder
}
...
func (b *ClusterUpgradeListBuilder) Copy(list *ClusterUpgradeList) *ClusterUpgradeListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*ClusterUpgradeBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewClusterUpgrade().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'cluster_upgrade' objects using the
// configuration stored in the builder.
func (b *ClusterUpgradeListBuilder) Build() (list *ClusterUpgradeList, err error) {
	items := make([]*ClusterUpgrade, len(b.items))
