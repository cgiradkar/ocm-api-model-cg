...
type TemplateParameterListBuilder struct {
	items []*TemplateParameterBuilder
}
...
func (b *TemplateParameterListBuilder) Copy(list *TemplateParameterList) *TemplateParameterListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*TemplateParameterBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewTemplateParameter().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'template_parameter' objects using the
// configuration stored in the builder.
func (b *TemplateParameterListBuilder) Build() (list *TemplateParameterList, err error) {
	items := make([]*TemplateParameter, len(b.items))
	...
	list = new(TemplateParameterList)
	list.items = items
	return
}
