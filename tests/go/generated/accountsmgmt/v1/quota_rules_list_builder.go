...
func (b *QuotaRulesListBuilder) Copy(list *QuotaRulesList) *QuotaRulesListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*QuotaRulesBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewQuotaRules().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'quota_rules' objects using the
// configuration stored in the builder.
func (b *QuotaRulesListBuilder) Build() (list *QuotaRulesList, err error) {
	items := make([]*QuotaRules, len(b.items))
