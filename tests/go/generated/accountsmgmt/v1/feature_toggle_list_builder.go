...
func (b *FeatureToggleListBuilder) Copy(list *FeatureToggleList) *FeatureToggleListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*FeatureToggleBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewFeatureToggle().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'feature_toggle' objects using the
// configuration stored in the builder.
func (b *FeatureToggleListBuilder) Build() (list *FeatureToggleList, err error) {
	items := make([]*FeatureToggle, len(b.items))
