...
// DefaultCapabilityListBuilder contains the data and logic needed to build
// 'default_capability' objects.
type DefaultCapabilityListBuilder struct {
	items []*DefaultCapabilityBuilder
}
...
func (b *DefaultCapabilityListBuilder) Items(values ...*DefaultCapabilityBuilder) *DefaultCapabilityListBuilder {
	b.items = make([]*DefaultCapabilityBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *DefaultCapabilityListBuilder) Copy(list *DefaultCapabilityList) *DefaultCapabilityListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*DefaultCapabilityBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewDefaultCapability().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'default_capability' objects using the
// configuration stored in the builder.
func (b *DefaultCapabilityListBuilder) Build() (list *DefaultCapabilityList, err error) {
	items := make([]*DefaultCapability, len(b.items))
	...
	list = new(DefaultCapabilityList)
	list.items = items
	return
}
