...
type PullSecretsRequestListBuilder struct {
	items []*PullSecretsRequestBuilder
}
...
func (b *PullSecretsRequestListBuilder) Copy(list *PullSecretsRequestList) *PullSecretsRequestListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*PullSecretsRequestBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewPullSecretsRequest().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'pull_secrets_request' objects using the
// configuration stored in the builder.
func (b *PullSecretsRequestListBuilder) Build() (list *PullSecretsRequestList, err error) {
	items := make([]*PullSecretsRequest, len(b.items))
	...
	list = new(PullSecretsRequestList)
	list.items = items
	return
}
