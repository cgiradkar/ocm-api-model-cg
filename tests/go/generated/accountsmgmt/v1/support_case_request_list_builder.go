...
// SupportCaseRequestListBuilder contains the data and logic needed to build
// 'support_case_request' objects.
type SupportCaseRequestListBuilder struct {
	items []*SupportCaseRequestBuilder
}
...
func (b *SupportCaseRequestListBuilder) Copy(list *SupportCaseRequestList) *SupportCaseRequestListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*SupportCaseRequestBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewSupportCaseRequest().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'support_case_request' objects using the
// configuration stored in the builder.
func (b *SupportCaseRequestListBuilder) Build() (list *SupportCaseRequestList, err error) {
	items := make([]*SupportCaseRequest, len(b.items))
	...
	list = new(SupportCaseRequestList)
	list.items = items
	return
}
