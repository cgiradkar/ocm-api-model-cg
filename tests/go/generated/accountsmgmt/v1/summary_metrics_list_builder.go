...
func (b *SummaryMetricsListBuilder) Copy(list *SummaryMetricsList) *SummaryMetricsListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*SummaryMetricsBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewSummaryMetrics().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'summary_metrics' objects using the
// configuration stored in the builder.
func (b *SummaryMetricsListBuilder) Build() (list *SummaryMetricsList, err error) {
	items := make([]*SummaryMetrics, len(b.items))
