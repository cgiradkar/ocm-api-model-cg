...
// ContractDimensionListBuilder contains the data and logic needed to build
// 'contract_dimension' objects.
type ContractDimensionListBuilder struct {
	items []*ContractDimensionBuilder
}
...
func (b *ContractDimensionListBuilder) Copy(list *ContractDimensionList) *ContractDimensionListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*ContractDimensionBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewContractDimension().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'contract_dimension' objects using the
// configuration stored in the builder.
func (b *ContractDimensionListBuilder) Build() (list *ContractDimensionList, err error) {
	items := make([]*ContractDimension, len(b.items))
	...
	list = new(ContractDimensionList)
	list.items = items
	return
}
