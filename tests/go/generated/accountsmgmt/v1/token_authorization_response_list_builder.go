...
// TokenAuthorizationResponseListBuilder contains the data and logic needed to build
// 'token_authorization_response' objects.
type TokenAuthorizationResponseListBuilder struct {
	items []*TokenAuthorizationResponseBuilder
}
...
func (b *TokenAuthorizationResponseListBuilder) Items(values ...*TokenAuthorizationResponseBuilder) *TokenAuthorizationResponseListBuilder {
	b.items = make([]*TokenAuthorizationResponseBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *TokenAuthorizationResponseListBuilder) Copy(list *TokenAuthorizationResponseList) *TokenAuthorizationResponseListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*TokenAuthorizationResponseBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewTokenAuthorizationResponse().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'token_authorization_response' objects using the
// configuration stored in the builder.
func (b *TokenAuthorizationResponseListBuilder) Build() (list *TokenAuthorizationResponseList, err error) {
	items := make([]*TokenAuthorizationResponse, len(b.items))
	...
	list = new(TokenAuthorizationResponseList)
	list.items = items
	return
}
