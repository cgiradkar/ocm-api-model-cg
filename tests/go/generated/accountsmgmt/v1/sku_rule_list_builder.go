...
func (b *SkuRuleListBuilder) Copy(list *SkuRuleList) *SkuRuleListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*SkuRuleBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewSkuRule().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'sku_rule' objects using the
// configuration stored in the builder.
func (b *SkuRuleListBuilder) Build() (list *SkuRuleList, err error) {
	items := make([]*SkuRule, len(b.items))
