...
// MarshalCloudAccountList writes a list of values of the 'cloud_account' type to
// the given writer.
func MarshalCloudAccountList(list []*CloudAccount, writer io.Writer) error {
	stream := helpers.NewStream(writer)
	writeCloudAccountList(list, stream)
	err := stream.Flush()
	if err != nil {
		return err
	}
...
// writeCloudAccountList writes a list of value of the 'cloud_account' type to
// the given stream.
...
// UnmarshalCloudAccountList reads a list of values of the 'cloud_account' type
...
func UnmarshalCloudAccountList(source interface{}) (items []*CloudAccount, err error) {
	...
	items = readCloudAccountList(iterator)
	err = iterator.Error
	return
}
...
func readCloudAccountList(iterator *jsoniter.Iterator) []*CloudAccount {
	...
	for iterator.ReadArray() {
		item := readCloudAccount(iterator)
