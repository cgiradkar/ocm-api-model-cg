...
func (b *EscalationListBuilder) Copy(list *EscalationList) *EscalationListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*EscalationBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewEscalation().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'escalation' objects using the
// configuration stored in the builder.
func (b *EscalationListBuilder) Build() (list *EscalationList, err error) {
	items := make([]*Escalation, len(b.items))
