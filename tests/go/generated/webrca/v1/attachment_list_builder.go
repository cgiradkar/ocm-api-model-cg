...
func (b *AttachmentListBuilder) Copy(list *AttachmentList) *AttachmentListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*AttachmentBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewAttachment().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'attachment' objects using the
// configuration stored in the builder.
func (b *AttachmentListBuilder) Build() (list *AttachmentList, err error) {
	items := make([]*Attachment, len(b.items))
