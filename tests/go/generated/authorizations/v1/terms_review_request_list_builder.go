...
type TermsReviewRequestListBuilder struct {
	items []*TermsReviewRequestBuilder
}
...
func (b *TermsReviewRequestListBuilder) Copy(list *TermsReviewRequestList) *TermsReviewRequestListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*TermsReviewRequestBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewTermsReviewRequest().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'terms_review_request' objects using the
// configuration stored in the builder.
func (b *TermsReviewRequestListBuilder) Build() (list *TermsReviewRequestList, err error) {
	items := make([]*TermsReviewRequest, len(b.items))
	...
	list = new(TermsReviewRequestList)
	list.items = items
	return
}
