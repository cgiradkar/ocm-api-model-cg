...
func (b *KubeletConfigListBuilder) Copy(list *KubeletConfigList) *KubeletConfigListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*KubeletConfigBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewKubeletConfig().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'kubelet_config' objects using the
// configuration stored in the builder.
func (b *KubeletConfigListBuilder) Build() (list *KubeletConfigList, err error) {
	items := make([]*KubeletConfig, len(b.items))
