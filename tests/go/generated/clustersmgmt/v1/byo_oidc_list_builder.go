...
func (b *ByoOidcListBuilder) Copy(list *ByoOidcList) *ByoOidcListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*ByoOidcBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewByoOidc().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'byo_oidc' objects using the
// configuration stored in the builder.
func (b *ByoOidcListBuilder) Build() (list *ByoOidcList, err error) {
	items := make([]*ByoOidc, len(b.items))
