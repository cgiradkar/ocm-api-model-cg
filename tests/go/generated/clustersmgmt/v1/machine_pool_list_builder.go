...
func (b *MachinePoolListBuilder) Copy(list *MachinePoolList) *MachinePoolListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*MachinePoolBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewMachinePool().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'machine_pool' objects using the
// configuration stored in the builder.
func (b *MachinePoolListBuilder) Build() (list *MachinePoolList, err error) {
	items := make([]*MachinePool, len(b.items))
