...
func (b *NodeInfoListBuilder) Copy(list *NodeInfoList) *NodeInfoListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*NodeInfoBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewNodeInfo().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'node_info' objects using the
// configuration stored in the builder.
func (b *NodeInfoListBuilder) Build() (list *NodeInfoList, err error) {
	items := make([]*NodeInfo, len(b.items))
