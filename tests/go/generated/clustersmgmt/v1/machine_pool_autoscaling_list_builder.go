...
type MachinePoolAutoscalingListBuilder struct {
	items []*MachinePoolAutoscalingBuilder
}
...
func NewMachinePoolAutoscalingList() *MachinePoolAutoscalingListBuilder {
	return new(MachinePoolAutoscalingListBuilder)
}
...
func (b *MachinePoolAutoscalingListBuilder) Items(values ...*MachinePoolAutoscalingBuilder) *MachinePoolAutoscalingListBuilder {
	b.items = make([]*MachinePoolAutoscalingBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *MachinePoolAutoscalingListBuilder) Copy(list *MachinePoolAutoscalingList) *MachinePoolAutoscalingListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*MachinePoolAutoscalingBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewMachinePoolAutoscaling().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'machine_pool_autoscaling' objects using the
// configuration stored in the builder.
func (b *MachinePoolAutoscalingListBuilder) Build() (list *MachinePoolAutoscalingList, err error) {
	items := make([]*MachinePoolAutoscaling, len(b.items))
	...
	list = new(MachinePoolAutoscalingList)
	list.items = items
	return
}
