...
func (b *IngressListBuilder) Copy(list *IngressList) *IngressListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*IngressBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewIngress().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'ingress' objects using the
// configuration stored in the builder.
func (b *IngressListBuilder) Build() (list *IngressList, err error) {
	items := make([]*Ingress, len(b.items))
