...
func (b *GCPVolumeListBuilder) Copy(list *GCPVolumeList) *GCPVolumeListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*GCPVolumeBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewGCPVolume().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'GCP_volume' objects using the
// configuration stored in the builder.
func (b *GCPVolumeListBuilder) Build() (list *GCPVolumeList, err error) {
	items := make([]*GCPVolume, len(b.items))
