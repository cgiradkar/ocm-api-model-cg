...
// CredentialRequestListBuilder contains the data and logic needed to build
// 'credential_request' objects.
type CredentialRequestListBuilder struct {
	items []*CredentialRequestBuilder
}
...
func (b *CredentialRequestListBuilder) Copy(list *CredentialRequestList) *CredentialRequestListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*CredentialRequestBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewCredentialRequest().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'credential_request' objects using the
// configuration stored in the builder.
func (b *CredentialRequestListBuilder) Build() (list *CredentialRequestList, err error) {
	items := make([]*CredentialRequest, len(b.items))
	...
	list = new(CredentialRequestList)
	list.items = items
	return
}
