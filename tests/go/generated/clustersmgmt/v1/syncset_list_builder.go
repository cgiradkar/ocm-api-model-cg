...
func (b *SyncsetListBuilder) Copy(list *SyncsetList) *SyncsetListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*SyncsetBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewSyncset().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'syncset' objects using the
// configuration stored in the builder.
func (b *SyncsetListBuilder) Build() (list *SyncsetList, err error) {
	items := make([]*Syncset, len(b.items))
