...
func (b *NodesInfoListBuilder) Copy(list *NodesInfoList) *NodesInfoListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*NodesInfoBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewNodesInfo().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'nodes_info' objects using the
// configuration stored in the builder.
func (b *NodesInfoListBuilder) Build() (list *NodesInfoList, err error) {
	items := make([]*NodesInfo, len(b.items))
