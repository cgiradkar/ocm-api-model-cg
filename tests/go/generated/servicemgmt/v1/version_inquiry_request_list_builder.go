...
// VersionInquiryRequestListBuilder contains the data and logic needed to build
// 'version_inquiry_request' objects.
type VersionInquiryRequestListBuilder struct {
	items []*VersionInquiryRequestBuilder
}
...
func (b *VersionInquiryRequestListBuilder) Copy(list *VersionInquiryRequestList) *VersionInquiryRequestListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*VersionInquiryRequestBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewVersionInquiryRequest().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'version_inquiry_request' objects using the
// configuration stored in the builder.
func (b *VersionInquiryRequestListBuilder) Build() (list *VersionInquiryRequestList, err error) {
	items := make([]*VersionInquiryRequest, len(b.items))
	...
	list = new(VersionInquiryRequestList)
	list.items = items
	return
}
