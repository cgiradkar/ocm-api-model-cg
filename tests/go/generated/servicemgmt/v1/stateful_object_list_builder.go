...
func (b *StatefulObjectListBuilder) Copy(list *StatefulObjectList) *StatefulObjectListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*StatefulObjectBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewStatefulObject().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'stateful_object' objects using the
// configuration stored in the builder.
func (b *StatefulObjectListBuilder) Build() (list *StatefulObjectList, err error) {
	items := make([]*StatefulObject, len(b.items))
