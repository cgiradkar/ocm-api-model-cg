...
// NewManagedServiceClient creates a new client for the 'managed_service'
...
type ManagedServicePollRequest struct {
	request    *ManagedServiceGetRequest
...
func (r *ManagedServicePollRequest) Predicate(value func(*ManagedServiceGetResponse) bool) *ManagedServicePollRequest {
	r.predicates = append(r.predicates, func(response interface{}) bool {
		return value(response.(*ManagedServiceGetResponse))
	})
	return r
}
...
type ManagedServicePollResponse struct {
	response *ManagedServiceGetResponse
}
...
func (r *ManagedServiceGetRequest) SendContext(ctx context.Context) (result *ManagedServiceGetResponse, err error) {
	...
	defer response.Body.Close()
	result = &ManagedServiceGetResponse{}
	result.status = response.StatusCode
	result.header = response.Header
	reader := bufio.NewReader(response.Body)
	_, err = reader.Peek(1)
	...
	err = readManagedServiceGetResponse(result, reader)
	if err != nil {
		return
	}
	return
