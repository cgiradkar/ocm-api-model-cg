...
func (b *LabelReferenceListBuilder) Copy(list *LabelReferenceList) *LabelReferenceListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*LabelReferenceBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewLabelReference().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'label_reference' objects using the
// configuration stored in the builder.
func (b *LabelReferenceListBuilder) Build() (list *LabelReferenceList, err error) {
	items := make([]*LabelReference, len(b.items))
