...
type ManagementClusterParentListBuilder struct {
	items []*ManagementClusterParentBuilder
}
...
func NewManagementClusterParentList() *ManagementClusterParentListBuilder {
	return new(ManagementClusterParentListBuilder)
}
...
func (b *ManagementClusterParentListBuilder) Items(values ...*ManagementClusterParentBuilder) *ManagementClusterParentListBuilder {
	b.items = make([]*ManagementClusterParentBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *ManagementClusterParentListBuilder) Copy(list *ManagementClusterParentList) *ManagementClusterParentListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*ManagementClusterParentBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewManagementClusterParent().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'management_cluster_parent' objects using the
// configuration stored in the builder.
func (b *ManagementClusterParentListBuilder) Build() (list *ManagementClusterParentList, err error) {
	items := make([]*ManagementClusterParent, len(b.items))
	...
	list = new(ManagementClusterParentList)
	list.items = items
	return
}
