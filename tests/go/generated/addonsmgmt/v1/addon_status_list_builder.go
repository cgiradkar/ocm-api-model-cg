...
func (b *AddonStatusListBuilder) Copy(list *AddonStatusList) *AddonStatusListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*AddonStatusBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewAddonStatus().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'addon_status' objects using the
// configuration stored in the builder.
func (b *AddonStatusListBuilder) Build() (list *AddonStatusList, err error) {
	items := make([]*AddonStatus, len(b.items))
