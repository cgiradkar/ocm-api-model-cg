...
type MetricsFederationListBuilder struct {
	items []*MetricsFederationBuilder
}
...
func (b *MetricsFederationListBuilder) Copy(list *MetricsFederationList) *MetricsFederationListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*MetricsFederationBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewMetricsFederation().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'metrics_federation' objects using the
// configuration stored in the builder.
func (b *MetricsFederationListBuilder) Build() (list *MetricsFederationList, err error) {
	items := make([]*MetricsFederation, len(b.items))
	...
	list = new(MetricsFederationList)
	list.items = items
	return
}
