...
// AddonInstallationParametersListBuilder contains the data and logic needed to build
// 'addon_installation_parameters' objects.
type AddonInstallationParametersListBuilder struct {
	items []*AddonInstallationParametersBuilder
}
...
func NewAddonInstallationParametersList() *AddonInstallationParametersListBuilder {
	return new(AddonInstallationParametersListBuilder)
}
...
func (b *AddonInstallationParametersListBuilder) Items(values ...*AddonInstallationParametersBuilder) *AddonInstallationParametersListBuilder {
	b.items = make([]*AddonInstallationParametersBuilder, len(values))
	copy(b.items, values)
	return b
}
...
func (b *AddonInstallationParametersListBuilder) Copy(list *AddonInstallationParametersList) *AddonInstallationParametersListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*AddonInstallationParametersBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewAddonInstallationParameters().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'addon_installation_parameters' objects using the
// configuration stored in the builder.
func (b *AddonInstallationParametersListBuilder) Build() (list *AddonInstallationParametersList, err error) {
	items := make([]*AddonInstallationParameters, len(b.items))
	...
	list = new(AddonInstallationParametersList)
	list.items = items
	return
}
