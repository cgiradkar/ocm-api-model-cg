...
// AddonStatusConditionListBuilder contains the data and logic needed to build
// 'addon_status_condition' objects.
type AddonStatusConditionListBuilder struct {
	items []*AddonStatusConditionBuilder
}
...
func (b *AddonStatusConditionListBuilder) Copy(list *AddonStatusConditionList) *AddonStatusConditionListBuilder {
	if list == nil || list.items == nil {
		...
		b.items = make([]*AddonStatusConditionBuilder, len(list.items))
		for i, v := range list.items {
			b.items[i] = NewAddonStatusCondition().Copy(v)
		}
	}
	return b
}

// Build creates a list of 'addon_status_condition' objects using the
// configuration stored in the builder.
func (b *AddonStatusConditionListBuilder) Build() (list *AddonStatusConditionList, err error) {
	items := make([]*AddonStatusCondition, len(b.items))
	...
	list = new(AddonStatusConditionList)
	list.items = items
	return
}
